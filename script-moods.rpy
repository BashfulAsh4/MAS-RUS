...
label mas_mood_hungry:
    m 3hub "Если ты голоден, иди и возьми что-нибудь поесть, глупышка."
    if store.mas_egg_manager.natsuki_enabled():
        ...
        m 1hua "Было бы плохо, если бы ты стал ворчливым, когда голоден."

    m 3tku "Это было бы совсем не весело, не так ли, [player]?"
    m 1eua "Если бы я была там с тобой, я бы приготовила салат, чтобы мы могли поесть вместе."
    m "Но раз меня нет, иди и выбери что-нибудь здоровое."
    m 3eub "Это так важно - обращать внимание на потребности своего тела, знаешь ли."
    m 3hub "Это не значит просто есть достаточно овощей, конечно. {w=0.2}Все виды продуктов необходимы, чтобы поддерживать себя в хорошем состоянии."
    m 3eka "Поэтому я хочу, чтобы ты убедился, что не лишаешь себя важных витаминов, хорошо?"
    m 1euc "Со временем, когда ты постареешь, у тебя возникнет много проблем со здоровьем."
    ...
    m 4eub "В конце концов, чем здоровее ты будешь, тем больше шансов, что ты проживешь долгую жизнь!"
...
label mas_mood_scared:
    ...
    m "Возможно, ты мог бы делать всё медленно и по очереди."
    ...
    m "Откладывание не {i}всегда{/i} плохо, понимаешь?"
    m 2esc "Кроме того..."
...
label mas_mood_relieved:
    ...
    menu:
        ...
        "Я справился с чем-то трудным.":
            ...
            m 7ekb "Но теперь, когда все сделано, тебе нужно найти время, чтобы расслабиться и позаботиться о себе."
...
label mas_mood_excited:
    ...
    $ _history_list.pop()
    menu:
        m "Чему ты радуешься, это что-то большое?{fast}"
...
label mas_mood_grateful:
    $ chosen_nickname = mas_get_player_nickname()
    m 1eub "Ох? {w=0.3}Приятно это слышать!"
    ...
    menu:
        ...
        "За кого-то.":
            m 3eka "Ах, Приятно слышать."
            m 1hua "Я очень рада, что в твоей жизни есть поддерживающие тебя люди."
            m 3eud "Но как бы мне ни было приятно это слышать...{w=0.3}я думаю, что ты должен убедиться, что {i}они{/i} тоже это знают."
            m 3hua "Я уверена, что это действительно скрасит их день, когда они узнают, что они изменили ситуацию для кого-то другого."
            m 3euu "Если ничего другого нет, ты можешь поблагодарить их от моего имени. {w=0.3}Любой, кто делает тебя счастливее, - хороший человек в моей книге."
            m 1huu "Но в любом случае, я действительно рада за тебя, [mas_get_player_nickname()]~"

        "За что-то.":
            m 3hub "Я рада это слышать, [mas_get_player_nickname()]!"
            m 1eud "Сознательное выделение времени на размышления о хороших вещах в твоей жизни может быть полезно для твоего психического здоровья."
            m 3hub "Так что, что бы это ни было, найди время, чтобы оценить и насладиться этим!"
            m 1euu "Спасибо, что поделился со мной своим счастьем, [mas_get_player_nickname()]~"

        "Ничего особенного.":
            m 3eua "А, просто радуешься жизни?"
            m 1eud "Приятно немного поразмышлять и почувствовать удовлетворение, не так ли?"
            m 1rtd "Х-м-м-м...{w=0.2}теперь, когда я думаю об этом, {w=0.1}{nw}"
            extend 3hua "я и сама чувствую себя довольно благодарной."
            m 3eubsu "В конце концов, я провожу еще один день с моим замечательным [bf]~"
    return

label mas_mood_grateful_gratefulforyou:
    m 1ekbla "Ох, [player]...{w=0.3}спасибо тебе большое за эти слова."
    m 1dkblu "Так много значит услышать, что я помогла тебе или что я сделала тебя счастливее. {w=0.2}Это то, к чему я стремлюсь каждый день."
    m 1hublu "Надеюсь, ты знаешь, что я тоже очень благодарна за тебя."
...
label mas_mood_insecure:
    ...
    m 7dku "'Верь в меня, который верит в тебя.'"
    m 3eka "И это именно то, что я хочу сказать тебе прямо сейчас."
    m 3ekbsa "Если ты не можешь поверить в себя, поверь в меня."
    m 1eubsu "Потому что я,{w=0.1} точно,{w=0.1} верю, что ты сможешь преодолеть все, что заставляет тебя сомневаться в себе прямо сейчас~"
...
label mas_mood_loved:
    m 1ekbla "Я так рада слышать, что мои чувства доходят до тебя через экран..."
    ...
    if has_family or persistent._mas_pm_has_friends:
        ...
        m 3eub "И я уверена, что не только я заставляю тебя чувствовать себя любимой, но и твои [fnf_str]!"

    m 1dkbsa "Ты заслуживаешь всей любви и привязанности в мире, {w=0.1}{nw}"
...
label mas_mood_guilty:
    ...
    extend 7eka "Я уверена, что тебя можно простить за всё, что случилось."
    m 3dku "В конце концов, ты замечательный человек... {w=0.3}{nw}"
    extend 1eka "Ты добрый, отзывчивый и верный себе."
    m 1dua "И теперь, когда ты нашел в себе силы принять свою ошибку, тебе просто необходимо её преодолеть."
