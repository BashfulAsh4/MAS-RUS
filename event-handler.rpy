...
label prompt_menu:
    ...
    python:
        ...
        if len(unseen_event_labels) > 0 and persistent._mas_unsee_unseen:
            ...
            mas_setEVLPropValues(
                "mas_show_unseen",
                prompt="Я бы хотел увидеть 'Неувиденное' ([unseen_num]) снова"
            )
        else:
...
label mas_bookmarks_unbookmark(bookmarks_items):
    ...
    if len(bookmarks_items) > 1:
        $ renpy.say(m, "Какую закладку ты хотел удалить?", interact=False)

    else:
        $ renpy.say(m, "Просто выбери закладку, если ты уверен, что хочешь удалить её.", interact=False)
