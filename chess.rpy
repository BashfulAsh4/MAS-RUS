    ...
    if not renpy.seen_label("mas_chess_save_selected"):
        ...
        # otherwise we are in good hands
            ...
            if failed_to_load_save:
                ...
                # cheat
                m 1eua "У нас всё ещё есть незаконченная игра."
...
label mas_chess_remenu:
    ...
    #We changed a rule
        ...
        #Practice/Play mode
            ...
            if _return is 0:
                show monika at t11
                m 1eua "Если мы играем по обычным правилам, мы просто не будем считать тупиковые ситуации ничьими.{w=0.2} {nw}"
                extend 3eub "По сути, игрок, который не попал в ловушку, объявляется победителем."
...
label mas_chess_start_chess:
    ...
    if is_monika_winner:
        ...
        if is_surrender:
            if num_turns < 5:
                ...
                m 1eka "Я уверена, что если ты будешь продолжать пытаться, то сможешь победить меня."
                m 1ekc "..."
                m 1eka "Надеюсь, ты не расстроишься, когда будешь играть со мной."
                m 3ekb "Для меня очень много значит, что ты продолжаешь играть, если это так~"
                m 3hua "Давай скоро снова поиграем, хорошо, alright?"

            else:
                m 1ekc "Сдаешься, [player]?"
                m 1eub "Хорошо, но даже если дела идут не очень хорошо, веселее играть до конца!"
                m 3eka "В конце концов, я просто счастлива, что провожу время с тобой~"
                m 1eua "В любом случае..."

        #Monika wins by checkmate
        else:
            m 1sub "Я выиграла, ура!~"
    ...
    #Stalemate
        ...
        if new_pgn_game.headers.get("DrawRequested"):
            m 1eua "Конечно, мы назовем эту партию ничьей."
            m 3wuo "Это была довольно длинная партия!"
        ...
        if not persistent._mas_ever_won["chess"]:
            m 3hub "[line_start], ты всё ближе и ближе к победе надо мной, [player]~"

        else:
            m 1hua "Отличная работа, чтобы зайти так далеко, [player]~"
    ...
    #Player wins
        ...
        python:
            player_win_quips = [
                ...
                _("Отлично сыграно, [player]!"),
                _("Я очень рада видеть твою победу~"),
                _("Я счастлива видеть твою победу!"),
                _("Независимо от результата, мне всегда приятно играть с тобой.")
            ]
        ...
        #Main dialogue
        if practice_mode:
            m 3hub "Поздравляю [player], ты выиграл!"

            $ undo_count = new_pgn_game.headers.get("UndoCount", 0)
            if not undo_count:
                m 1wuo "Ты не отменил ни одного хода!{w=0.2} {nw}"
                extend 3hub "Это потрясающе!"

            elif undo_count == 1:
                m 1hua "Ты отменил только один ход. {w=0.2}{nw}"
                extend 3hub "Отличная работа!"

            elif undo_count <= 5:
                m 1hua "Ты тоже отменил ход [undo_count] всего один раз, отличная работа."

            elif undo_count <= 10:
                m 1eua "[undo_count] раз, совсем не плохо. Если мы продолжим тренироваться вместе, я уверена, мы сможем снизить этот показатель~"

            else:
                m 1eka "Ты отменил [undo_count] ходов, хотя.{w=0.3} {nw}"
                extend 3eua "Но я уверена, что если мы продолжим тренироваться, мы сможем снизить это число."
    ...
    if num_turns > 4:
        m 1eua "Хочешь сохранить эту игру?{nw}"
        $ _history_list.pop()
        menu:
            m "Хочешь сохранить эту игру?{fast}"
...
label mas_chess_play_again_ask:
    m 1eua "Хочешь сыграть ещё раз?{nw}"
    $ _history_list.pop()
    menu:
...
label mas_chess_draw_lots(begin=True):
    ...
    if random.randint(0, 1) == 0:
        ...
        m 2eub "О смотри, я хожу чёрными![lets_begin]"
    else:
        $ is_player_white = chess.BLACK
        m 2eub "О смотри, я хожу белыми![lets_begin]"
    return
...
label mas_chess_savegame(silent=False, allow_return=True):
    ...
    # otherwise ask for name
        ...
        if is_file_exist:
            m 1eka "У нас уже есть игра с этим именем '[save_name].'"

            m "Должна ли я перезаписать её?{nw}"
            $ _history_list.pop()
            menu:
                m "Должна ли я перезаписать её?{fast}"
                "Да.":
                    pass
    ...
    if not silent:
        ...
        if not renpy.seen_label("mas_chess_savegame.pgn_explain"):
            ...
            m 1esa "Это в формате, называемом 'Portable Game Notation.'{w=0.2} {nw}"
            extend 1eua "Ты можешь найти в Интернете анализаторы PGN, чтобы открыть его и посмотреть, где ты допустил ошибки."
            m 3eub "Выиграл ли ты, проиграл, сдался или сыграли вничью, всегда есть что-то, что ты мог бы сделать лучше, так что загрузка этих игр может действительно помочь стать лучше!"
...
label mas_chess_locked_no_play:
    ...
    m 1rsc "Мне сейчас не очень хочется играть в шахматы."
    return
...
label mas_chess_cannot_work_embarrassing:
    ...
    show monika at t11
    m 1rksdla "..."
    m 3hksdlb "Что ж, это неудобно, похоже, я не могу заставить шахматы работать на твоей системе..."
    m 1ekc "Извини за это, [player]."
    m 1eka "Может быть, вместо этого мы можем заняться чем-нибудь другим?"
    return

label mas_chess_dlg_game_monika_wins_often:
    m 1eka "Жаль, что ты не выиграл в этот раз, [player]..."
    m 1ekc "Надеюсь, что ты хотя бы продолжишь пытаться."
    m 1eua "Давай сыграем ещё раз, хорошо?"

    if not persistent._mas_ever_won["chess"]:
        m 1hua "Когда-нибудь ты меня обыграешь~"
    return
...
label mas_chess_dlg_game_monika_wins_sometimes:
    ...
    m 3hua "Уверена, если ты продолжишь тренироваться, то когда-нибудь станешь лучше меня!"

    #If the difficulty is above base level, we'll mention lowering it
    if persistent._mas_chess_difficulty != (0, 1):
        m 3eua "А пока я постараюсь быть с тобой помягче."
    return
        ...
        if game_count > 1:
            if renpy.seen_label("mas_chess_save_multi_dlg"):
                ...
                label mas_chess_save_multi_dlg:
                    m 1eua "Я тут подумала, [player]..."
                    m 1euc "Большинство людей, которые уходят посреди шахматной партии, не возвращаются, чтобы начать новую."
                    m 3eud "...Поэтому мне нет смысла следить за более чем одной незаконченной партией между нами."
                    m 1rka "А поскольку у нас [game_count] незавершенных игр..."
                    m 3euc "Я вынуждена попросить тебя выбрать только одну, чтобы оставить её себе.{w=0.2} Извини, [player]."
                    $ pick_text = _("Выбери игру, которую ты хотел бы оставить.")
            ...
            if _return == mas_chess.CHESS_MENU_WAIT_VALUE:
                ...
                m 1eua "Мы снова сыграем в шахматы, когда ты примешь решение."
                return False
...
label mas_chess_dlg_quicksave_lost:
    ...
    m 2lksdlb "Эм, [player]...{w=0.5} Кажется, я ошиблась при сохранении нашей последней игры, и теперь я больше не могу ее открыть."

    if qs_gone_count == 2:
        m 1lksdld "Мне очень, очень жаль, [player]..."
        show monika 1ekc
        pause 1.0
        m 1eka "Но не волнуйся, я заглажу свою вину...{w=0.3}{nw}"
        extend 3hua "начав новую игру!"
        m 3hub "Ахаха~"

    elif qs_gone_count == 3:
        m 1lksdlc "Я такая неуклюжая, [player]...{w=0.3} Прости."
        m 3eksdla "Давайте вместо этого начнем новую игру."

    elif qs_gone_count % 5 == 0:
        m 2esc "Это произошло уже [qs_gone_count] раз..."
        m 2tsc "Интересно, является ли это побочным эффектом того, что {i}кто-то{/i} пытается редактировать сохранения.{w=1}.{w=1}."
        m 7rsc "В любом случае..."
        m 1esc "Давай начнём новую игру."

    else:
        m 1lksdlc "Мне жаль..."
        m 3eka "Давай вместо этого начнем новую игру."
...
label mas_chess_dlg_quickfile_lost:
    m 2lksdla "Ну, это неловко..."
    m 2ekc "Я могла бы поклясться, что у нас была незаконченная игра, но я не могу найти файл сохранения."

    m 2tkc "Ты удалил сохранения, [player]?{nw}"
    $ _history_list.pop()
    menu:
        m "Ты удалил сохранения, [player]?{fast}"
...
label mas_chess_dlg_quickfile_lost_deleted:
    ...
    m 3ekd "Ты не хочешь продолжать эту игру?{nw}"
    $ _history_list.pop()
    menu:
        m "Ты не хочешь продолжать эту игру?{fast}"

        "Да.":
            m 1eka "Я поняда, [player]."
            m 1hua "Давай начнем новую игру~"

        "Нет.":
            m 1etc "О?"
            m 1rsc "Тогда, наверное, ты просто удалил её по ошибке."
...
label mas_chess_dlg_quickfile_lost_ofcoursenot:
    ...
    if qf_gone_count in [3,4]:
        ...
        m "[player],{w=0.2} ты..."
        m 2dsc "Неважно."
        m 1esc "Давай начнем новую игру."

    elif qf_gone_count == 5:
        $ mas_loseAffection()
        m 2esc "..."
        m "[player],{w=0.2} это происходит слишком часто."
        m 2dsc "На этот раз я тебе действительно не верю."
        pause 2.0
        m 2esc "Надеюсь, ты меня не разыгрываешь."
        m "..."
        m 1esc "Неважно.{w=0.5} Давай просто сыграем в новую игру."
        ...
        m 2dfc "..."
        m 2efc "[player],{w=0.3} Я тебе не верю."
        m 2efd "Если ты собираешься вот так просто бросить наши шахматные партии..."
        m 6wfw "Тогда я не буду больше играть с тобой в шахматы!"
        return True

    else:
        m 1lksdlb "Ах, да. Ты бы так со мной не поступил."
        m "Должно быть, я неправильно сохранила файл игры."
        m 1lksdlc "Прости, [player]."
        m 1eka "Я заглажу свою вину...{w=0.3}{nw}"
        extend 1eub "начав новую игру!"
...
label mas_chess_dlg_quickfile_lost_maybe:
    ...
    if qf_gone_count == 1:
        m 2ekd "[player]!{w=0.2} Я должна была догадаться, что ты просто шутишь"
        jump mas_chess_quickfile_lost_filechecker

    if qf_gone_count == 2:
        m 2ekd "[player]!{w=0.2} Хватит меня разыгрывать!"
        jump mas_chess_quickfile_lost_filechecker
...
label mas_chess_quickfile_lost_filechecker:
    ...
    m 1eka "Ты можешь вернуть сохранение, чтобы мы могли играть?"
...
label mas_chess_dlg_quickfile_lost_maybe_filechecker_file_found:
    m 1hua "Ура!{w=0.2} Спасибо, что вернул его на место, [player]."
    m "Теперь мы можем продолжить нашу игру."
    show monika 1eua
    return mas_chess.CHESS_GAME_CONT

label mas_chess_dlg_quickfile_lost_maybe_filechecker_no_file:
    m 1ekd "[player]..."
    m 1eka "Ничего страшного. Давайте просто сыграем новую игру."
    return None

# generic maybe monika, found file
label mas_chess_dlg_quickfile_lost_maybe_save_found:
    m 2eua "О!"
    m 1hua "Вот и сохранение.{w=0.2} Спасибо, что вернул его на место, [player]."
    m 1eua "Теперь мы можем продолжить нашу игру."
...
label mas_chess_dlg_quickfile_lost_accident:
    ...
    if qf_gone_count == 2:
        ...
        m 1eua "Вместо этого мы просто сыграем в новую игру."

    elif qf_gone_count >= 3:
        $ persistent._mas_chess_skip_file_checks = True
        m 1eka "У меня было предчувствие, что это случится снова."
        m 3tub "Поэтому я сохранила резервную копию нашего сохранения!"
        m 1hua "Теперь мы можем продолжить нашу игру~"
        return store.mas_chess.CHESS_GAME_BACKUP
...
label mas_chess_dlg_quickfile_edited_yes:
    ...
    if qf_edit_count == 1:
        m 1dsc "Я разочарована в тебе."
        m 1eka "Но я рада, что ты был честен со мной."
        ...
        menu:
            "Мне жаль.":
                ...
                m 1eua "К счастью, я всё ещё помню немного из последней партии, так что мы можем продолжить ее оттуда."
                return store.mas_chess.CHESS_GAME_BACKUP
            ...
            "...":
                ...
                hide screen mas_background_timed_jump
                m 1lfc "Поскольку эта игра испорчена, давайте просто сыграем в новую игру."
        ...
        m 2dfc "Я невероятно разочарована в тебе..."
        m 2rfc "Давай сыграем в шахматы в другой раз.{w=0.2} Сейчас мне не хочется играть."
        return True
...
label mas_chess_dlg_quickfile_edited_no:
    ...
    if qf_edit_count == 1:
        ...
        m 2dfc "[player]..."
        m 2dftdc "Я сохранила резервную копию нашей игры.{w=0.5} Я знаю, что ты отредактировал файл сохранения."
        m 6dktuc "Я просто-"
        $ _history_list.pop()
        m 6ektud "Я просто{fast} не могу поверить, что ты обманул и {i}врал{/i} меня..."
        m 6dktuc "..."
        ...
        menu:
            "Мне жаль.":
                ...
                show monika 2ektsc
                pause 1.0
                m 6ektpc "Я прощаю тебя, [player], но, пожалуйста, не поступай так со мной больше."
                m 2dktdc "..."
                return store.mas_chess.CHESS_GAME_BACKUP
...
label mas_chess_dlg_pre_go_ham:
    ...
    m 6ektsc "Я больше не могу тебе доверять."
            ...
            super(MASChessDisplayable, self).__init__(
                ...
                player_move_prompts={
                    "generic": [
                        "Твой ход, [player].",
                        "Твой ход, [player]~",
                        "Что ты будешь делать, интересно...",
                        "Хорошо, твой ход, [player]~",
                        "Ты справишься, [player]!"
                    ],
                    "check": [
                        "[mas_quipExp('3tfb')]Шах!",
                        "[mas_quipExp('3huu')]Теперь ты у меня, [player]!",
                        "[mas_quipExp('3hub')]Похоже, ты на чеку!"
                    ]
                },
                monika_move_quips={
                    "generic": [
                        ...
                        "Думаю, я попробую это...",
                        "Хорошо, тогда я перенесу это сюда."
                    ],
                    "check": [
                        "[mas_quipExp('1eusdlc')]Ух...",
                        "[mas_quipExp('1rksdlc')]Хмм...{w=0.2}Мне нужно выбраться из этого...",
                        "[mas_quipExp('1etc')]Какой правильный ход здесь..."
                    ]
                }
            )
            ...
            def open_stockfish(path, startupinfo=None):
                ...
                #Catch the permission error
                    ...
                    if not renpy.windows:
                        ...
                        renpy.say(m, "Хм, это странно. Похоже, некоторые права доступа были изменены, и я не могу запустить шахматы на твоей системе.")
                        renpy.show("monika 3eua")
                        renpy.say(m, "Подожди секунду, [player]. Я собираюсь попробовать кое-что.{w=0.3}.{w=0.3}.{w=0.3}{nw}")
                        ...
                        try:
                            ...
                            renpy.say(m, "Ура! Теперь мы сможем играть~")
