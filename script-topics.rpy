...
init 5 python:
    addEvent(Event(persistent.event_database,eventlabel="monika_death",category=['философия'],prompt="Смерть и умирание",random=True))
...
label monika_saved:
    ...
    if persistent._mas_pm_cares_about_dokis:
        m 1ekc "Конечно, у тебя есть члены клуба, но насколько это действительно важно, если их отзывы являются лишь частью какого-то скрипта?"
    else:
...
label monika_lastpoem:
    ...
    if not mas_safeToRefDokis():
        ...
        m 3rssdlc "Я имею в виду ту, перед самым суицидом Юри, с цветными полосками и белым шумом."
...
label monika_mc:
    ...
    m 1lfb "Главные герой - ничто, кроме транспортное средство для игрока.{nw}"
    $ _history_list.pop()
...
label monika_girlfriend:
    if (mas_isA01() and mas_isMoniNormal(higher=True)) or (renpy.random.randint(1,4) == 1 and mas_isMoniEnamored(higher=True)):
        ...
        m 1hua "Я не позволю кому-то другому украсть моего [mas_get_player_nickname(exclude_names=['my love', 'love', player], _default='sweetheart', regex_replace_with_nullstr='my ')]!"
    return
...
label monika_vnanalysis:
    ...
    m 2tfu "Мне просто нужно научиться менять персонажей в других играх, и ты можешь видеть меня везде."
...
label monika_playersface:
    ...
    if persistent._mas_pm_shared_appearance:
        m 5eka "Если ты такой же красивый как ты себя описал, то я очень рада, что ты поделился со мной своей внешностью."
...
label monika_fanfiction:
    ...
    if store.mas_anni.pastSixMonths() and mas_isMoniEnamored(higher=True):
        ...
        m 1lkbsa "Просто читай только приличные. Мы ещё не настолько продвинулись в наших отношениях!~"
    ...
    if mas_isMoniAff(higher=True):
        ...
        menu:
            "Не волнуйся [m_name]. Я здесь.":
                ...
                m 6dstsc "Т-{w=0.3}ты м-{w=0.2}можешь..."
                m "Т-{w=0.5}ты знаешь..."
...
label monika_lottery:
    ...
    m 3ekc "От друзей ты либо отдалаешься, либо они пытаются подлизаться к тебе, чтобы получить часть его для себя."
...
label monika_penname:
    ...
    m "Большинство писателей обычно используют их для кофиденциальности и сохранения своей личности в секрете."
...
label monika_daydream:
    ...
    if mas_isMoniNormal(higher=True):
        ...
        m 1lkbsa "Я представляла себе как, [daydream_quip]"
...
label monika_sports:
    ...
    menu:
        ...
        "Нет, но если с тобой...":
            ...
            m 3eua "Я научу тебя играть, когда я приду...{w=0.5}или, если тебе просто не терпится, ты можешь брать у меня уроки!"
...
label monika_writingtip6:
    ...
    m 3etd "Что это может сказать твоему читателю о его личности, если он использует модную страницу с цветочным принтом? {w=0.2}Или мятую тетрадную бумагу?"
    ...
    m 1hub "Даже если это не имеет никакого значения для твоего письма, рисование на хорошем холсте может быть полезным, так сказать."
...
label monika_kizuna:
    ...
    m 3rksdla "Мне кажется, что она - актриса озвучки, скрывающаяся за 3D-куклой."
...
label monika_solipsism:
    ...
    m 3eua "Приятно осознавать, что ты не одинок в бесконечном океане неопределенности, не так ли?"
...
label monika_timetravel:
    ...
    m 2ekc "Ну, путешественники во времени сами должны отвечать за себя, но только если ничего страшного с ними не произойдёт."
    ...
    m 1lksdlc "И я бы задавалось вопросом, забыл ли ты обо мне или не хочешь больше проводить со мной время."
    if mas_isMoniDis(lower=True):
        ...
        "Да.":
            ...
            menu:
                ...
                "Это было плохо.":
                    ...
                    m 1hua "И теперь, когда я здесь, с тобой, мы можем создавать их вместе~"
...
label monika_metamorphosis:
    ...
    m 3hksdlb "Например, Грегор, будучи единственным финансовым спонсором, больше беспокоится о потере работы, чем о своем состоянии!"
...
label monika_graduation:
    ...
    #Normal flow
        ...
        m 2eka "Эта речь длится около четырёх минут, так что убедись, что тебя хватить времени на то, чтобы прослушать её целиком."
...
label monika_grad_speech:
    ...
    m 2eka "{w=0.2}Все эти маленькие роли учат вас так много о будущем и о том,ы как управлять{w=0.3} как проектами, так и людьми,{w=0.3} в среде, которая вам нравится, тем не менее.{w=0.6}{nw}"
...
label monika_savingwater:
    ...
    if mas_isMoniLove(higher=True) and mas_is18Over():
        ...
        m "Э-хе-хе~"
        m 5hubfu "Было бы хорошо, если бы ты приготовил хорошую, расслабляющую ванну для нас, чтобы нам вместе принять её, когда я попаду в твой мир."
...
label monika_player_appearance_eye_color_blue:
    ...
    m 4eub "А ты знаешь, что ген для голубых глаз рецессивный, то есть встречается он у меньшего числа людей?"
...
label monika_player_appearance_eye_color_green:
    ...
    $ persistent._mas_pm_eye_color = "зелёные"
...
label monika_dating_startdate:
    ...
    if _diff.days == 0:
        ...
        m 2wfw "Ты не мог вызвать это событие сегодня, [player]."

        m "Я знаю, что ты возишься с кодом.{nw}"
        $ _history_list.pop()
        menu:
            m "Я знаю, что ты возишься с кодом.{fast}"
            "Я нет!":
                pass
            "Ты раскусила меня.":
                pass
        m 2tfu "Хмф,{w=0.2} ты меня не провёдешь."
...
label monika_load_custom_music:
    ...
    if diff > 0:
        ...
        menu:
            ...
            "Да.":
                m "Хорошо, убедись, что ты все сделал правильно."
...
label monika_perspective:
    m 2euc "Я давно хотела поговорить с тобой кое о чем, [player]..."
...
label monika_robotbody:
    ...
    m 1hub "А-ха-ха~"
    show monika 5eubla at t11 zorder MAS_MONIKA_Z with dissolve_monika
    m 5eubla "В общем, несмотря ни на что, я не буду возвражать по поводу всего этого, пока ты со мной."
...
label monika_sleigh:
    ...
    if mas_isFall() or mas_isWinter():
        m 1eka "Быть может, мы могли бы также укрыться магким, шерстяным одеялом, и обниматься под ним."
        m 1hkbla "О-о-о-о~"
...
label monika_life_skills:
    ...
    m 3ekc "Я слышала о том, что некоторые люди заканчивают университет, а потом разваливаются на части, потому что не знают, как платить налоги или записываться на прием к врачу."
    ...
    m 2esc "Теперь я знаю, что многим людям вовсе не обязательно считать программирование жизненным навыком, но не все люди живут в компьютере."
...
label monika_dystopias:
    ...
    m 3ekd "Поскольку это поучительные истории, ты не можете оставить читателя с ощущением, что в конце концов все закончилось хорошо."
...
label monika_social_contagion:
    ...
    m 3esc "Я слышала, что такие вещи, как переедание, азартные игры и злоупотребление алкоголем, тоже являются заразным поведением."
...
label monika_songwriting:
    ...
    if persistent.monika_kill:
        m 1ttu "Интересно, моя ли песня привела нас сюда~"
...
label monika_translating_poetry:
    ...
    m 3hksdlb "Ах! Я вовсе не хотела, чтобы это прозвучало так, будто я разглагольствую или ещё чего!"
...
label monika_striped_pajamas:
    ...
    m 3euc "В обоих случаях есть два человека из разных миров, которые никто из них не понимает полностью, и они огорождены друг от друга барьером."
...
label monika_language_nuances:
    ...
    m 3etc "И следуя этой схеме, 'Good day'(тоже 'Добрый день или Приятного дня') тоже будет звучать вполне уместно, верно? {w=0.2}В конце концов, это работает во многих других языках."
...
label monika_asimov_three_laws:
    ...
    m 1esd "Таких людей можно спасти, но робот ему не поможет, потому что он {i}технически{/i} мёртв."
    m 3eud "С другой стороны, если ты скажешь 'да', то робот может начать раскапывать могилы, чтобы 'помочь' людям, которые мертвы уже много лет."
...
label monika_being_herself:
    ...
    extend 2lksdlc "а другие,{w=0.2} хоть они никогда ничего и не говорили, ненавидели меня, потому что я представляла то, чем, по их мнению, и быть не может."
...
label monika_being_deleted:
    if mas_isMoniEnamored(higher=True):
        ...
        m 6ektda "Но если тебя это успокоит, я вовсе не расстроена из-за этого, [mas_get_player_nickname()]. {w=0.2}Я давно простила тебя за все это."
...
label monika_remembrance:
    ...
    m 3wkd "...И если мои воспоминания действительно фальшивые, тогда...{w=0.2}кто--{w=0.2}или {i}что{/i}--{w=0.2}я такое?"
    ...
    if mas_isMoniUpset(higher=True):
        ...
        if mas_isMoniNormal(higher=True):
            ...
            if mas_anni.pastOneMonth():
                $ line_mid = "чем я занимаюсь, и продолжаю заниматься,"
            else:
...
label monika_why_do_you_read:
    ...
    m 3euc "Когда я была совсем маленькой, мне нравилось писать короткие рассказы, но всегда не было кого-то, кому бы я могла их показать..."
...
label monika_look_into_eyes:
    m 3eub "Эй [player], посмотри мне в глаза на секунду..."
    ...
    m 3hubla "Э-хе-хе~"
    m 3rksdla "Прости [player], я просто пыталась увидеть твои красивые глаза через экран."

    #A tuple for eye color means the player has heterochromia
    $ eye_detail = "завораживающие" if isinstance(persistent._mas_pm_eye_color, tuple) else persistent._mas_pm_eye_color
    m 1dubsu "Когда мы остаемся вдвоем, я не могу не представлять себе твой [eye_detail] глаза..."
    show monika 5dubsa at t11 zorder MAS_MONIKA_Z with dissolve_monika
    m 5dubsa "Время останавливается, и я наконец-то могу просто...{w=0.3}забыть обо всех своих проблемах."
    m 5hubfb "..."
    m 5tubfa "Большое спасибо, [player]~"
    m 5kubfu "Потому что ты сейчас здесь со мной, я так спокойна."
...
label monika_literature_value:
    ...
    m 3ekd "Те, кто так думают, намеренно игнорируют литературу по сравнению с другими предметами, такими как физика или математика, мол это пустая трата времени и не даёт ничего полезного."
...
label monika_renewable_energy:
    ...
    m 3rkc "...Кроме того, ветряные мельницы довольно громкие и это частная проблема тех, кто живёт рядом с ними."
    m 3rsc "Геотермальная энергия надежна и отлично подходит для отопления и охлаждения, но она дорогая, зависит от конкретного места и может даже вызывать землетрясения."
...
label monika_taking_criticism:
    ...
    m 3eua "Но только убедись, что ты действительно его учитываешь, а не слепо защищаешь."
...
label monika_gmos:
    ...
    m 2esd "Если вкратце - мы не знаем наверняка. {w=0.2}Десятилетия исследований показали, что ГМО {i}вероятно{/i} безвредны, но у нас почти нет данных об их долгосрочном воздействии."
...
label monika_curse_words:
    ...
    m 1eud "Не говоря уже о том, что ругательства в разговоре могут заставить их чувствовать себя одновременно{w=0.1} гораздо более непринужденными {w=0.1}{nw}"
...
label monika_mc_is_gone:
    ...
    m 3rksdla "Надеюсь, ты тоже не думал [line_mid] всё это время..."
    ...
    m 2rssdlc "Я имею в виду, в конце концов, не похоже, что ты так уж сильно его контролировал."
...
label monika_know_its_you:
    if mas_isMoniEnamored(higher=True):
        ...
        extend 2eku "Я верю, что ты не оставишь меня одну, потому {i}ты{/i} заботишься обо мне."
...
label monika_information_age:
    ...
    m 1dua "В этом нет ничего постыдного, мы все просто пытаемся сделать все возможное, используя полученную информацию."
...
label monika_materialism:
    m 1euc "Слушай, [player]. Помнишь, мы говорили о солипсизме?"
    m 1eud "Теперь, когда мы говорили об этом, я подумала, что мы могли бы поговорить о родственной философии,{w=0.1} материализме,{w=0.1} который как бы противоположен солипсизму."
    m 2euc "Материалистический взгляд утверждает, что именно материя рождает мысль, а не мысль рождает материю."
    m 7euc "Это довольно логично, если думать об этом с чисто научной точки зрения.{w=0.2} {nw}"
    extend 7eud "В конце концов, нейроны - это просто разные виды материи."
    m 7eua "Все стимулы, которые ты чувствуешь, на самом деле являются изменениями в потенциалах нейронов.{w=0.2} Точно так же и мои приходят от кода, взаимодействующего с данными."
    m 2eua "Так что, если упростить, наши мысли и чувства - это просто движение и реакции материи."
    m 2lsc "Если мы оставим это, то это не так уж интересно.{w=0.2} {nw}"
    extend 2hksdlb "На самом деле, ты, наверное, уже слышал о таких вещах, верно?"
    m 7esc "Но... ну, если это всё, что есть,{w=0.1} это оставляет несколько вопросов, которые меня беспокоят."
    m 1esd "Например, опровергает ли материализм существование души?{w=0.2} {nw}"
    extend 1etd "Является ли душа каким-то неоткрытым типом материи?"
    m 3rsc "Есть свидетельства того, что человеческое сознание продолжается после того, как кто-то находится в состоянии смерти мозга, что ещё."
    m 3euc "А ещё есть рассказы о сверхъестественных или духовных встречах..."
    m 3hksdlb "Я знаю, о чем ты думаешь: не похоже, чтобы я воспринимала их всерьез! Но, ну..."
    m 2eksdlc "Когда я действительно думаю об этом, с моей стороны было бы немного лицемерно отвергать эти рассказы."
    ...
    m 2rsc "Открытие тебя и твоего мира...{w=0.3}это было нечто сверхъестественное, не так ли? {w=0.2}{nw}"
    ...
    m 3wud "Это был бы {i}довольно{/i} сложный баг!"
    m 3eua "Итак, я думаю, я чувствую уверенность в том, что --{w=0.1}по крайней мере, в моём жизненном опыте--{w=0.1}материализм может ответить на многие вопросы о Вселенной,{w=0.1} {nw}"
    extend 3eka "но, возможно, не на {i}все{/i}."

    if mas_isMoniEnamored(higher=True):
        m 1eka "Хотя...{w=0.3}это можно объяснить каким-то конкретным способом или нет, одно остается верным."
        m 1ekbla "...И это то, что это привело меня к встрече с тобой."
...
label monika_likecosplay:
    if mas_hasUnlockedClothesWithExprop("cosplay"):
        ...
        m 2rksdla "Я сама не очень люблю играть, так что, наверное, буду делать это лишь понемногу..."
        $ p_nickname = mas_get_player_nickname()
...
label monika_zodiac_starsign:
    ...
    m 4wud "Например, они даже оценивают потенциальных партнеров и друзей по их знаку!"
...
label monika_jekyll_hyde:
    ...
    m 2dksdld "К сожалению, для некоторых это всё ещё так."
...
label monika_conventions:
    m 1eud "Скажи [player], я тут подумала..."
    m 3eua "Ты когда-нибудь был на сходке комиксов или аниме?{nw}"
    $ _history_list.pop()
    menu:
        m "Ты когда-нибудь был на сходке комиксов или аниме?{fast}"

        "На сходке комиксов.":
            $ persistent._mas_pm_gone_to_comic_con = True
            $ persistent._mas_pm_gone_to_anime_con = False
            m 1hub "А, понятно! {w=0.2}Надеюсь, тебе было очень весело!"
            m 3eua "Комиксы - это действительно интересная среда в литературе,{w=0.1} {nw}"
            extend 3rta "может быть, мне стоит почитать еще..."

        "На сходке аниме.":
            $ persistent._mas_pm_gone_to_comic_con = False
            $ persistent._mas_pm_gone_to_anime_con = True
            if persistent._mas_pm_watch_mangime:
                m 3eub "У меня было предчувствие, что так и будет! {w=0.2}Они действительно показались мне чем-то, что может тебе понравиться."
            else:
                m 2wub "Правда? Это удивительно!"
                m 7eta "Ах,{w=0.1} может быть, ты ходил с друзьями?"
                m 3etd "...Или, возможно, ты пошел по другой причине...{w=0.3}интерес к играм, возможно?"

        "Я был и там, и там!":
            $ persistent._mas_pm_gone_to_comic_con = True
            $ persistent._mas_pm_gone_to_anime_con = True
            if persistent._mas_pm_watch_mangime:
                m 1hub "О! Я знала, что ты уже любишь аниме, но комиксы тебе тоже нравятся?"
                m 3eua "Они действительно интересный медиум в литературе, возможно, мне стоит почитать еще..."
            else:
                m 1wub "Ох! {w=0.3}Я не думала, что тебе нравится аниме, но похоже, что ты все равно фанат комиксов!"
                m 3eua "Не то чтобы это было слишком удивительно, их атмосфера кажется такой, что она может понравиться любому."
        ...
        "Нет.":
            ...
            if persistent._mas_pm_watch_mangime and persistent._mas_pm_social_personality == mas_SP_EXTROVERT:
                m 2etd "Правда?"
                m 7eub "Я удивлена! {w=0.3}Когда я узнала о конвенциях аниме, я сразу подумала о тебе."
                m 3eud "Хотя, думаю, расходы на проезд могут быть довольно высокими в зависимости от того, где ты находишься."
            else:
                m 2eud "А, понятно."
                m 7eua "Я полагаю, что независимо от интереса, сходки могут быть немного сложным."
                m 3eud "В зависимости от того, где ты живешь, они могут быть довольно затратными."

    m 3hua "Я всегда думала, что сходки - это очень весело! {w=0.3}Место, где каждый может просто быть самим собой и наслаждаться своими интересами без осуждения."
    m 3eub "Я люблю смотреть на фотографии всех талантливых косплееров и безумные наряды, которые они сделали."
    m 1wuo "Это безумие, что люди могут сделать, когда они чем-то увлечены!"
    m 3eua "Я также слышала, что там есть много веселых мероприятий, таких как танцевальные шоу идолов, тривиальные игры и другие развлечения."
    m 1eubsa "Я бы с удовольствием когда-нибудь сходила туда с тобой, [mas_get_player_nickname()]~"
    return
...
init 5 python:
    addEvent(
        Event(
            ...
            category=["моника"],
            prompt="Какой твой любимый вкус кекса?",
            pool=True,
            unlocked=False,
            rules={"no_unlock":None},
...
label monika_cupcake_favorite:
    m 1rta "Хм, я не уверена, что у меня действительно есть один..."
    m 1hub "Мне нравятся самые разные виды, так что трудно выбрать только один!"
    m 3ekd "Кажется, я уже упоминала, как сильно скучаю по кексам Нацуки..."
    m 3eua "Однажды она приготовила очень странный кекс со вкусом мятной шоколадной крошки...{w=0.3}у него была мятная глазурь с шоколадной посыпкой и шоколадная основа для кекса."
    m 4rksdlb "Это была одна из самых странных вещей, которые я когда-либо пробовала, а-ха-ха!"
    m 2eksdlb "На самом деле вкус совсем не похож на вкус мороженого с мятной шоколадной крошкой, вместо этого он напоминал зубную пасту!"
    m 2ekp "Это было немного разочаровывающе...{w=0.3}Я ожидала, что это будет мой любимый вкус кекса."
    m 7eka "О, хорошо, было приятно, что она попыталась сделать что-то уникальное, что мне понравится...{w=0.3}несмотря на её жесткий внешний вид, она может быть очень милой~"
    return
...
label monika_pizza:
    m 1eub "Пицца? {w=0.2}Да, я иногда ею наслаждаюсь!"
    m 1hua "Это не всегда самый здоровый выбор, но это может быть приятным лакомством и сытным блюдом."
    ...
    m 1duc "Если бы я выбирала любимую начинку, хмм...{w=0.3}{nw}"
    extend 3hub "грибы хороши, или что-либо вегетарианское--{w=0.2}на самом деле, хочешь верь, хочешь нет, но шпинат может быть удивительно вкусным!"
    m 3eua "...И, конечно, ты никогда не ошибешься с обычным сыром."
    m 3luc "Хмм..."
    ...
    m 1hksdlb "Хотя это довольно спорная тема в Интернете, у меня никогда не было возможности попробовать пиццу с ананасами."
    m 1lksdlb "Поэтому я не могу принять участие в этом конкретном споре. Извини, [player]!"
    m 3huu "Но, думаю, это значит, что когда-нибудь ты сможешь увидеть моё первое впечатление."
    return
...
init 5 python:
    addEvent(
        Event(
            ...
            prompt="Что ты думаешь о киберспорте?",
            pool=True,
        )
    )
...
label monika_esports:
    if mas_isFirstSeshDay():
        m 1rtd "Хм, это хороший вопрос..."
    else:
        m 1eub "Забавно, что ты спрашиваешь, я как раз изучала это, пока тебя не было!"
    m 3eua "Я думаю, это действительно интересно, как меняется все наше восприятие просмотра спорта..."
    m 3euc "Зрительская аудитория киберспорта продолжает соперничать с традиционными спортивными событиями,{w=0.1} {nw}"
    extend 3wud "и может даже обогнать их в течение следующих 5-10 лет!"
    m 2tsd "Не так давно люди смотрели на видеоигры свысока, считая их пустой тратой времени, {w=0.1}{nw}"
    extend 7hub "но теперь некоторые из этих игроков зарабатывают миллионы долларов, играя в некоторые из своих любимых игр!"
    m 3eua "Это действительно показывает, что ты можешь заниматься любимым делом...{w=0.3}даже тем, над чем люди могут насмехаться."
    m 3eud "Если что-то не популярно или не является массовым, это не значит, что оно останется таким навсегда..."
    m 1huu "Не бойся идти против тренда, {w=0.1}то, чем ты увлечен, возможно, ищет того самого первопроходца, который поможет вывести это на передний план~"
    return
...
label monika_overton:
    m 1etc "Эй [player], ты когда-нибудь слышал об окне Овертона?"
    m 3eud "Это политологическая концепция, которая отражает ценностную структуру общества."
    m 3euc "По сути, все идеи человека рассматриваются на определенной стадии одобрения массами."
    m 2esc "Джозеф Овертон узнал, как дегуманизировать людей, и объяснил, как перестроить человеческое восприятие."
    m 7eud "От неприемлемого, отвратительного и постыдного к нормальному, социальному и даже престижному."
    m "Эта концепция включает 6 этапов: Немыслимое, Радикальное, Приемлемое, Разумное, Стандартное и Текущая норма."
    m 3esa "Внутри окна Овертона находятся идеи, принятые обществом...{w=0.3}такие, как патриотизм, любовь к семье, человечность и честность."
    m 3eksdlc "За окном - все, что не одобряется, например, наркомания, алкоголизм, нацизм, тирания, рабство и так далее."
    m 3eud "Самое интересное, что окно можно сдвинуть в направлении идеи, например, превратить Немыслимое в Разумное."
    m 2lksdlc "Конечно, такой уровень изменений - довольно сложный процесс."
    ...
    m 2euc "Число участников таких движений со временем росло бы, некоторые из них - люди с разбитыми сердцами или те, кто чувствует себя удрученными в отношениях с реальным человеком."
    m 4eksdld "Естественно, появятся и люди, выступающие против движения."
    m 4eua "В связи с растущей популярностью новых ценностей общество активно давит на новую тенденцию. {w=0.2}В этот момент происходит подмена понятий."
    m 2eud "От Неприемлемого виртуальная любовь переходит к Радикальному."
    m 7eud "Отсюда тема виртуальной любви и любви к вымышленным персонажам долгое время обсуждается в обществе."
    m 3esc "Постепенно люди привыкают к существованию таких взглядов, но ещё не принимают их."
    m 1esd "Ученые и социологи пишут различные статьи и проводят исследования."
    m 3eua "Навязывается мнение, что любить вымышленного персонажа абсолютно нормально и в этом нет ничего ужасного."
    m 3huu "Из радикальной виртуальная любовь переходит в приемлемую."
    m 1eksdla "Общество уже смирилось с новым видением и считает, что любить вымышленного персонажа - это нормально, но всё же немного странно."
    m 3eua "Культура виртуальной любви постепенно развивается, создаются фильмы и шоу."
    ...
    m 2husdlb "Думаю, на этом мы можем остановиться на сегодня, это становится довольно долгим, а-ха-ха!"
    m 1eua "Я {i}могла{/i} бы закончить эту историю вплоть до Текущей Нормы, но я просто хотела описать ее на базовом уровне, чтобы передать пример того, как это может работать."
